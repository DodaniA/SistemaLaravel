<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gradient-orange">Citas con Pacientes</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
                Gestiona las citas que tienes programadas para tus pacientes
            </p>
        </div>
    ...
    @if($citas->isEmpty())
        <div class="rounded-xl border-2 border-dashed border-orange-200 dark:border-slate-700 bg-orange-50/50 dark:bg-slate-800/30 px-8 py-12 text-center">
            ...
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Cuando los pacientes agenden, aparecerán aquí</p>
        </div>
    @else
        <div class="grid gap-4">
            @foreach($citas as $cita)
                <div class="stat-card stat-card-orange group overflow-hidden">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <h3 class="font-semibold text-slate-800 dark:text-slate-100">
                                        {{ $cita->paciente->user?->name ?? 'Paciente ' . $cita->paciente_id }}
                                    </h3>
                                </div>
                            </div>
                    ...
                    <div class="px-6 py-3 border-t border-orange-100 dark:border-slate-700 bg-orange-50/30 dark:bg-slate-900/30 flex justify-end gap-2">
                    
                        @if($cita->estado === 'pendiente')
                            <flux:button
                                wire:click="atender({{ $cita->id }})"
                                variant="primary"
                                size="sm"
                            >
                                Atender
